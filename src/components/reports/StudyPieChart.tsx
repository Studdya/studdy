...
const StudyPieChart = ({
  ...
  const data: ChartData[] = subjects.map(subject => {
    const totalDuration = getSessionsByMonth(month, year).filter(session => session.subject.id === subject.id).reduce((total, session) => total + session.duration, 0);
    return {
      name: subject.name,
      value: totalDuration / 3600,
      // Convert to hours for better chart display
      color: subject.color,
  ...
  return <Card>
      <CardHeader className="pb-2">
        <CardTitle className="tracking-tight flex items-center gap-2 font-medium text-2xl">
      ...
      <CardContent className="h-[300px] pt-4">
        <ChartContainer config={chartConfig} className="h-full">
          <PieChart>
            <Pie data={data} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={2} dataKey="value" nameKey="name" labelLine={false} label={renderCustomizedLabel}>
              {data.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} stroke="transparent" strokeWidth={2} />)}
            </Pie>
            <Tooltip content={({
            active,
            payload
            ...
            if (active && payload && payload.length) {
              ...
              return <div className="rounded-lg border bg-background p-2 shadow-md">
                      <div className="mb-1 flex items-center gap-1.5" style={{
                  color: data.color
                }}>
            ...
            <Legend layout="vertical" verticalAlign="middle" align="right" iconType="circle" iconSize={8} formatter={(value, entry, index) => {
            const {
              payload
            } = entry as any;
